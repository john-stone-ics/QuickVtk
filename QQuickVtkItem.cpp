...
QSGNode* QQuickVtkItem::updatePaintNode(QSGNode* node, UpdatePaintNodeData*)
{
    ...
    if (dirtySize) {
        ...
        if (auto fb = n->vtkWindow->GetDisplayFramebuffer(); fb && fb->GetNumberOfColorAttachments() > 0) {
            ...
            auto *texture = window()->createTextureFromNativeObject(QQuickWindow::NativeObjectTexture, &texId, 0, sz.toSize(), static_cast<QQuickWindow::CreateTextureOptions>(QQuickWindow::TextureHasAlphaChannel));
#else
            auto *texture = QNativeInterface::QSGOpenGLTexture::fromNative(texId, window(), sz.toSize(), QQuickWindow::TextureIsOpaque | QQuickWindow::TextureHasAlphaChannel);
#endif
